...
# Don't save first face as it could be blurred if just entering the scene
count = 10
...
#    cv.WaitKey(5000) # For Mac camera to wake up [1]
    ...
    if capture:
        ...
        while not face_saved:
            ...
            if not frame:
                # Was a wait of 0 and continue was break,
                # but give Mac cam a chance to wake up [1]
                cv.WaitKey(10)
                continue
            if not frame_copy:
        ...
        if faces:
            if args.box:
                for ((x, y, w, h), n) in faces:
                    ...
                    # so scale the bounding box of each face and
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Take a photo containing a face with your webcam after "
        "an random interval. If no face was found, wait another random "
        "interval and try again. Tip: schedule this to run daily.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        '-i', '--camera_index', type=int, default=0,
        help="Camera index. Usually 0 but try other integers.")
#     parser.add_argument('-b', '--begin', type=int, default=1,
#         help="Photo to begin at")
    parser.add_argument(
        '-d', '--dir', default="/Users/hugo/Dropbox/images/faces/",
        help="Directory to save photos")
    parser.add_argument(
        '-c', '--cascade',
        help="Haar cascade file",
        default='/Users/hugo/macports/share/OpenCV/haarcascades/'
    ...
    parser.add_argument(
        '--random_interval', type=int, default=3,
        help="Max hours to wait before taking a photo")
    parser.add_argument(
        '--retry_interval', type=int, default=1,
        help="Max hours to wait between attempts at taking a photo, "
